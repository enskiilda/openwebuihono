...
Usuń całkowicie z kodu na stałe zależności ollama, openai, audio oraz inne zbędne elementy z kodu aplikacji frontendu by zoptymalizować go i nie był aż taki ogromny całkowicie usuń poprzednie wersje, historię edycji, martwy kod, śmieci, nieużywane elementy, zbędne elementy etc oraz całkowicie usuń LOGOWANIE AUTH KONTA UŻYTKOWNIKA oraz loading screen splash screen dodatkowo napraw błąd który powoduje, że frontejd się bardzo długo inicjalizuje bezwzględnie zawsze Pisz tylko i wyłącznie czysty kod, absolutnie nigdy nie wstawiaj markerów konfliktów <<<<<<<, =======, >>>>>>>⚠️ UWAGA! KRYTYCZNY KOMUNIKAT DOTYCZĄCY HARDCODOWANYCH API! ⚠️
...
Optymalizuj kod frontendu by nie był taki ogromny, ale nie zmieniając nawet minimalnie jego designu
